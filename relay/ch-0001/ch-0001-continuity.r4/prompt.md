...
- 只输出 `reviews/ch-0001.continuity.r4.md` 的完整 markdown 报告本体；不要寒暄，不要写文件，不要执行命令。
- 你可以且只能只读下列输入文件。
...
复审模式：只核销 r3 问题 C-R3-001，并抽查是否引入新硬事实问题。

必查：
1. `drafts/ch-0001.md` 是否已删除「五行驳杂」。
2. 清册判词是否仍只使用已登记的「至贱命格」。
3. 是否仍无「青槐镇」「七日」「赵四」「有意思了」等已知问题。
4. provenance 是否为 `ch-0001-prose.r4`。

## 输出格式

首行必须为 `VERDICT: pass | revise`，并包含 `ATTEMPT: r4`、`ISSUES`、`SEAM_CHECK`、`REPORT: reviews/ch-0001.continuity.r4.md`。
