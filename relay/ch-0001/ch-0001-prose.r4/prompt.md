...
- 你是 prose-writer。本条消息 + 下列输入文件 = 你的全部上下文；输出一次给全。
- 只输出 `drafts/ch-0001.md` 的完整 markdown 文件内容；不要寒暄，不要解释，不要写文件，不要执行命令。
- 你可以且只能只读下列输入文件。
...
按 revision brief 修订正文：只删除未登记判词「五行驳杂」，保留 r3 其他内容。输出完整 `drafts/ch-0001.md` 文件内容，末尾 provenance 改为：

`created_by: prose-writer@deepseek-v4-pro via opencode · ch-0001-prose.r4`
